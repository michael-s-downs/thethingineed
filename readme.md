...
* **window_overlap** (optional): Integer value to specify the window overlapping for text chunking purposes, 10 by default.
...
* **method** (optional): Type of chunking technique (<i>surrounding_context_window</i>, <i>recursive</i> or <i>simple</i> (by default)) that will be used to split normal chunks into smaller chunks to improve the retrieval process. Each chunking method has different parameters associated.
* **sub_window_length (<i>recursive method parameter</i>)** (required method) Integer value to specify the window length for the sub-chunks.
