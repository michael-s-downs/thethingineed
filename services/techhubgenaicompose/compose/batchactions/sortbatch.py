...
class BatchSortMethod:
    ...
    def __init__(self, streambatch: list) -> None:
        ...
        if len(streambatch) <= 1:
            raise DolffiaError(status_code=500, message="Cant sort 1 or less streambatch")
...
class BatchSortFactory:
    ...
    def __init__(self, batchsort_type: str) -> None:
        ...
        if self.batchsortmethod is None:
            raise DolffiaError(status_code=404, message=f"Provided batchsort does not match any of the possible ones: {', '.join(f.type for f in self.MERGEBATCHES)}")
