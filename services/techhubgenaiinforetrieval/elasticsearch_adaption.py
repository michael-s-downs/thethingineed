"""Elasticsearch vector store. This module is an adaption of the original ElasticsearchVectorStore
because in the original module you cannot use multiple filters in one query to do de retrieval.

Other feature added, is to get the scores of the documents in the query in haystack like format
"""
...
def _to_elasticsearch_filter(standard_filters: MetadataFilters) -> Dict[str, Any]:
    """
    Convert standard filters to Elasticsearch filter. Adaption to do Multiple filters in one query
    ...
    for filter in standard_filters.filters:
        ...
        for subfilter in filter.filters:
            operands_should.append({"term": {f"metadata.{subfilter.key}.keyword": subfilter.value}})
    ...
    async def aquery(
        ...
        **kwargs: Any,
        ...
        return VectorStoreQueryResult(
            nodes=top_k_nodes,
            ids=top_k_ids,
            similarities=self._to_haystack_similarities(top_k_scores),
        )
