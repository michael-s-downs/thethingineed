...
def get_embed_model(model: dict, aws_credentials: dict, is_retrieval: bool) -> BaseEmbedding:
    ...
    if platform == 'azure':
        ...
        if is_retrieval:
            ...
            return HuggingFaceEmbedding(model_name=model.get('retriever_model'), trust_remote_code=True, device="cpu" )
        return HuggingFaceEmbedding(model_name=model.get('embedding_model'), trust_remote_code=True, device="cpu")
