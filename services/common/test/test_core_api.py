...
mock_async_preprocess_template = {
    "timeout_sender": 0,
    "dataset_conf": {"dataset_path": "", "dataset_csv_path": "", "dataset_id": ""},
    "url_sender": "",
    "force_ocr": False,
    "origins": {"ocr": ""},
    "preprocess_conf": {},
...
@patch("provider_resources.queue_url", "mocked_queue_url")
@patch("genai_sdk_services.storage.StorageController")
def test_async_preprocess_request_generate_with_process_id(mock_storage, mock_generate_dataset, mock_file):
    """Test coverage for the new process_id lines"""
    from core_api import _async_preprocess_request_generate
    mock_generate_dataset.return_value = "mocked_dataset_path"
...
@patch("provider_resources.queue_url", "mocked_queue_url")
@patch("genai_sdk_services.storage.StorageController")
def test_async_preprocess_request_generate_with_preprocess_conf(mock_storage, mock_generate_dataset, mock_file):
    """Test coverage for preprocess_conf branch"""
    from core_api import _async_preprocess_request_generate
    mock_generate_dataset.return_value = "mocked_dataset_path"
    ...
    request_params = {
        "timeout": 30,
        "folder": "/test_folder",
        "preprocess_conf": {"some_config": "value"},  
        "integration": {"key": "value"},
        "tracking": {"id": "tracking_id"}
    }
    request_files = ["file1.jpg", "file2.jpg"]
    ...
    assert result["dataset_conf"]["dataset_path"] == "/test_folder"
    assert result["dataset_conf"]["dataset_csv_path"] == "mocked_dataset_path"
    assert result["preprocess_conf"]["some_config"] == "value"
    assert result["integration"] == {"key": "value"}
    assert result["tracking"] == {"id": "tracking_id"}
    assert result["url_sender"] == "mocked_queue_url"
...
def mock_env(monkeypatch):
    ...
    monkeypatch.setenv("CORE_SYNC_PREPROCESS_URL", "sync-preprocess.example.com")
    monkeypatch.setenv("CORE_SYNC_CLASSIFY_URL", "sync-classify.example.com")
    monkeypatch.setenv("CORE_SYNC_EXTRACT_URL", "sync-extract.example.com")
    monkeypatch.setenv("CORE_ASYNC_PROCESS_URL", "async-process.example.com")
    monkeypatch.setenv("CORE_QUEUE_PROCESS_URL", "queue-process.example.com")
...
def test_queue_preprocess_request(mock_set_credentials, mock_queue_write_message, mock_env):
    from core_api import queue_preprocess_request
    ...
    with patch("core_api._async_preprocess_request_generate") as mock_generate:
        mock_generate.return_value = {"dataset_conf": {"dataset_id": "test_id"}}
        result = queue_preprocess_request(apigw_params, request_params, request_files)

    assert result["status"] == "waiting"
    assert result["process_id"] == "test_id"
...
@patch("provider_resources.queue_write_message")
@patch("provider_resources.qc.set_credentials")
def test_queue_preprocess_request_error(mock_set_credentials, mock_queue_write_message, mock_env):
    from core_api import queue_preprocess_request
...
def test_queue_indexing_request(mock_set_credentials, mock_queue_write_message, mock_env):
    ...
    with patch("core_api._async_indexing_request_generate") as mock_generate:
        mock_generate.return_value = {"dataset_conf": {"dataset_id": "test_id"}}
...
@patch("requests.post")
def test_async_status_request_finished(mock_post, mock_env):
    from core_api import async_status_request
    mock_post.return_value = MagicMock(status_code=200, json=lambda: {"process_status": "finished"})
...
def test_async_delete_request_error(mock_post, mock_env):
    from core_api import async_delete_request
    mock_post.return_value = MagicMock(status_code=400, json=lambda: {"status": "error"})
...
@patch("requests.post")
def test_sync_delete_request_error(mock_post, mock_env):
    from core_api import sync_delete_request
    mock_post.return_value = MagicMock(status_code=400, json=lambda: {"status": "error"})
...
def test_async_status_request_error(mock_post, mock_env):
    from core_api import async_status_request
    mock_post.return_value = MagicMock(status_code=200, json=lambda: {'process_status': 'unknown_status'})
...
@patch("requests.post")
def test_async_result_request_empty_results_error(mock_post, mock_env):
    from core_api import async_result_request
    mock_post.return_value = MagicMock(status_code=200, json=lambda: {"info": None})
