...
class TsuzumiPlatform(Platform):
    ...
    def call_model(self, delta=0) -> dict:
        ...
        try:
            ...
            if delta < self.num_retries:
                if answer.status_code == 429:
                    self.logger.warning(
                        f"Tsuzumi rate limit exceeded, retrying, try {delta + 1}/{self.num_retries}"
                    )
                    raise ConnectionError("Tsuzumi rate limit exceeded")
    ...
    def build_url(self, generative_model: GenerativeModel):
        ...
        """
        self.logger.debug("Building url.")
        if generative_model.MODEL_MESSAGE in ["chatTsuzumi"]:
            url = self.models_urls.get('TSUZUMI_CHAT_URL')
            self.logger.debug("url: %s", url)
        else:
            raise PrintableGenaiError(
                400, f"Model message {generative_model.MODEL_MESSAGE} not supported."
...
class ManagerPlatform(object):
    MODEL_TYPES = [OpenAIPlatform, AzurePlatform, BedrockPlatform, VertexPlatform, TsuzumiPlatform]
