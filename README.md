...
- [Overview](#overview)
    - [Building Block overview](#building-block-overview)
...
- [Release notes](#release-notes)
    - [Update and change info](#update-and-change-info)
- [Getting started](#getting-started)
    ...
    - [Pricing Consideration](#pricing-consideration)
        - [Sandbox Deployment: Free of charge for limited hours.](#sandbox-deployment-free-of-charge-for-limited-hours)
        - [Local Deployment: Approximate monthly cost for main services of supported hyperscaler.](#local-deployment-approximate-monthly-cost-for-main-services-of-supported-hyperscaler)
            - [Azure hyperscaler (Japan East)](#azure-hyperscaler-japan-east)
            - [AWS hyperscaler (Tokyo)](#aws-hyperscaler-tokyo)
    ...
    - [Best Practice](#best-practice)
        ...
        - [Recommended Tools and Configuration Strategies](#recommended-tools-and-configuration-strategies)
        - [Maintenance and Observability](#maintenance-and-observability)
- [APIs](#apis)
...
- [Interface Guide](#interface-guide)
    - [Langfuse](#langfuse)
        - [Accessing Projects in Langfuse](#accessing-projects-in-langfuse)
        - [Inside a Project](#inside-a-project)
        - [Prompt Overview](#prompt-overview)
        - [Prompt Content and Tags](#prompt-content-and-tags)
        - [Versioning and Changes](#versioning-and-changes)
        - [Uploading a Template](#uploading-a-template)
        - [Retrieving a Template](#retrieving-a-template)
        - [Deleting a Template](#deleting-a-template)
        - [Re-uploading a Template](#re-uploading-a-template)
...
The RAG toolkit is a comprehensive Retrieval-Augmented Generation (RAG) system, featuring data preprocessing, Elasticsearch indexing, and API orchestration for efficient data retrieval and language model integration. It is composed of several components that are divided into 2 pipelines:
...
The Global RAG system is built on a modular, cloud-native architecture that separates indexing, retrieval, and language model orchestration. Each component is designed to operate independently and communicate via APIs and queues, ensuring scalability, flexibility, and easy integration with enterprise environments.

- COMPOSE

![alt text](documentation/media/Architecture_compose.png "Compose architecture")

The COMPOSE layer is responsible for orchestrating the logic required to resolve a user query. It coordinates retrieval, LLM execution, and template-based response composition.

genai_compose acts as the central orchestrator. It receives user queries via HTTP REST API and applies the configured Compose template logic.

It interacts with:

- `genai_inforetrieval`, which performs vector and metadata-based retrieval over indexed documents.

- `genai_llmapi`, which calls the configured large language model (LLM) through Azure, AWS Bedrock, or other providers.

All interactions happen via stateless HTTP calls, and Redis is used for caching and session state management if needed.

This modular design allows the system to support retrieval-only, LLM-only, or combined RAG (Retrieval-Augmented Generation) flows depending on the template selected.

- INDEXING AND FLOWMGMT

![alt text](documentation/media/Architecture_indexing.png "Indexing architecture")

This layer is responsible for ingesting, preprocessing, and indexing documents into the vector store. It is composed of three main subsystems:

**1. Integration Layer**  
`integration-receiver`: Accepts documents and metadata through REST API calls.

`integration-sender`: Forwards validated requests into the pipeline via queues.

**2. Flow Management**
Coordinates the document processing lifecycle using Redis and queues.

Subcomponents include:

`flowmgmt-infodelete`: Manages deletion of indexed content.

`flowmgmt-checkend` / `checktimeout`: Ensure consistency and timeout handling for long-running processes.

**3. Preprocessing Pipeline**
Processes documents in the following stages:

`preprocess-start`: Initializes metadata and routing.

`preprocess-extract`: Extracts raw content (text, tables).

`preprocess-ocr`: Applies OCR if needed.

`preprocess-end`: Finalizes and forwards the result to the indexer.

Each component communicates via dedicated queues to ensure modularity and scalability.

**4. Indexation Process**
`genai_infoindexing`: Embeds, chunks, and stores processed documents into the vector database, using configurable models (e.g., OpenAI ADA or Cohere).

Supports metadata enrichment and creation of new indices as needed.

This architecture ensures decoupled, asynchronous, and highly scalable document processing using cloud-native technologies like Azure, AWS, Redis, and managed queues.
...
- Compose
    ...
    * <i>techhub_retrieval</i>: to only retrieve chunks / documents, without content generation. This template does not use a LLM.
    ...
    Please note that the compose template being used is: **techhub_retrieval_reference**. This templates performs a number of actions that consist of retrieving the documents (using the parameter "query") from the "index" (where documents have been indexed) and then use the LLM component. The LLM component calls the specified "model" from the specified "platform" (model must be available in platform). The prompt sent to the LLM is defined by the parameter "template_name". In this case, we use the template: rag_with_references. This templates integrates the "query" and the retrieved context from documents and send it to the LLM. The available LLM templates are described in <i>LLM prompt templates</i> section.
...
Organizations can leverage RAG technology to automate knowledge discovery, improve decision-making, and enhance operational efficiency. The following use case scenarios demonstrate how the RAG building blocks can effectively transform information management and decision-making processes:

- **Internal knowledge assistant**
  
    Organizations often struggle with scattered information across multiple documents, databases, and repositories, making it difficult for employees to quickly find relevant information to perform their tasks effectively. The RAG system addresses this challenge by creating a unified, intelligent knowledge base that transforms how teams access and utilize organizational knowledge.

    **Key Benefits:**

    - **Centralized Knowledge Access**: Index internal documents, manuals, policies, procedures, and training materials into a searchable vector database with custom metadata for precise categorization.

    - **Intelligent Query Processing**: Natural language queries enable employees to ask complex questions like "What are the security protocols for handling customer data in remote work scenarios?" instead of manually searching through multiple policy documents.

    - **Multi-Document Synthesis**: Retrieve and combine information from multiple sources to provide comprehensive answers that span different departments or document types.

    - **Advanced Filtering and Retrieval**: Use metadata-based filtering to narrow searches by department, document type, creation date, author, or custom business categories.

    - **Query Expansion**: Automatically expand queries to include related terms and concepts, improving search accuracy across different languages and terminology variations.
        ...
        "generic": {
            "compose_conf": {
                "template": {
                    ...
                    "params": {
                        "query": "What are our data retention policies for European customers?",
                        "index": "company_policies",
                        "filters": {
                            "filename": "GDPR_Policy_2024.pdf"
                        },
                        "top_k": 10,
                        "top_qa": 5
                    }
...
- **Customer support automation**
  ...
  Customer support teams face the challenge of providing accurate, consistent responses while handling high volumes of inquiries across multiple channels and languages. The RAG system transforms static support documentation into an intelligent, dynamic assistant that enhances both agent productivity and customer satisfaction.

  **Key Benefits:**

    - **Instant Knowledge Retrieval**: Support agents can quickly access relevant information from product manuals, troubleshooting guides, FAQs, and historical support cases using natural language queries

    - **Multi-Source Information Synthesis**: Combine information from different sources (product docs, known issues, solution databases) to provide comprehensive solutions.

    - **Consistent and Accurate Responses**: Generated answers are based on official documentation and updated knowledge bases, ensuring consistency across different support agents and reducing human error.

    - **Query Expansion and Language Support**: Automatically expand customer queries to find relevant solutions even when using different terminology, and support multiple languages for global customer bases.

    - **Context-Aware Filtering**: Filter results by product version, customer tier, geographic region, or issue severity to provide targeted solutions.
        ...
        "generic": {
            "compose_conf": {
                "template": {
                    ...
                    "params": {
                        "query": "Customer can't log in after password reset",
...
- **Summarization and Reporting**
  ...
  Organizations need to extract actionable insights from large volumes of documents for strategic decision-making, compliance reporting, and operational efficiency. The RAG system enables sophisticated document analysis, automated report generation, and intelligent content synthesis that transforms how businesses process and understand their information assets.

    **Key Benefits:**

    - **Dynamic Report Generation**: Create comprehensive reports by combining retrieved information from multiple sources with intelligent sorting, grouping, and merging capabilities.

    - **Trend and Pattern Detection**: Use advanced querying to identify patterns across document collections, such as recurring issues in customer feedback or emerging market trends in industry reports.

    - **Compliance and Risk Assessment**: Generate compliance reports by systematically querying relevant regulations, policies, and audit documents with proper source attribution.

    - **Executive Intelligence**: Produce executive-level summaries with customizable length, focus areas, and presentation formats suitable for different stakeholder needs.

    - **Temporal Analysis**: Sort and group documents by date to track changes over time and identify emerging trends or compliance gaps.
        ...
        "generic": {
            "compose_conf": {
                "template": {
                    "name": "techhub_retrieval_reference",
                    "params": {
                        "query": "Analyze risk factors and mitigation strategies mentioned in all quarterly reports",
...
- **azure**:
  - **model**: Name of the model. On the Azure platform, this refers to the *deployment name* of the model, not the base model name.
  - **model_type**: Logical grouping of similar models, defined by the user. All deployments using the same underlying model (e.g., `gpt-4`) should share the same `model_type`.
  - **max_input_tokens**: Maximum number of input tokens accepted by the model. This depends on the model version and must be configured accordingly.
  - **zone**: Azure region where the model has been deployed. This is used to retrieve the appropriate API key from the secrets file.
  - **api_version**: The specific API version to be used when calling the model. Required for Azure OpenAI endpoints.
  - **model_pool**: List of logical pools or groups this model belongs to, for routing or scheduling purposes.

- **bedrock**:
  - **model**: Name of the model instance. Unlike Azure, this refers to the model as referenced in AWS Bedrock.
  - **model_id**: Unique identifier provided by Bedrock for the model (e.g., `anthropic.claude-v2`).
  - **model_type**: Same usage as Azure – should be consistent across platforms for similar models.
  - **max_input_tokens**: Same as above – determined by the specific model's token limit.
  - **zone**: AWS region (e.g., `us-east-1`) where the model is accessed.
  - **model_pool**: Same usage as above.

- **vertex**:
  - **model**: Name or alias of the model. For Google Vertex AI, this often refers to the published name of the model on the platform.
  - **model_id**: Internal ID or full resource path to the model (e.g., `gemini-1.5-pro-002`).
  - **model_type**: Same usage as above.
  - **max_input_tokens**: Same as above.
  - **zone**: GCP region (e.g., `us-central1`) for the deployed model.
  - **model_pool**: Same usage as above.

- **tsuzumi**:
  - **model**: Identifier or label of the model provided by the tsuzumi platform.
  - **model_type**: Same usage as above.
  - **max_input_tokens**: Same as above.
  - **model_pool**: Same usage as above.
...
|tsuzumi-7b-v1_2-8k-instruct| techhubdev-pool-world-tsuzumi-7b-v1_2-8k-instruct, techhub-pool-world-tsuzumi-7b-v1_2-8k-instruct                                 |tsuzumi|
*<i>A pool of models is a group of the same models allocated in different servers from a specific region, such as Europe or the US, that allows a more balanced deployment of models.</i>
...
Again in this config file, each model (separated by platforms) needs different parameters:
- **azure**:
...
All necessary credentials for the components are stored in secrets for security reasons. These secrets are JSON files that must be located under a common path defined by the environment variable 'SECRETS_PATH'; the default path is "secrets/". Within this secrets folder, each secret must be placed in a specific subfolder (these folder names are predefined). The different secrets that are used in the components are:
...
- Hyperscaler limited to Azure and AWS
- Vectorial Database limited to ElasticSearch and Azure AI Search
- OCR limited to Azure, GCP, AWS (Textract not available in some regions like Tokyo)
- Some LLM/Embeddings models not available in all regions
...
LLm supports all languages, but the templates are prepared for:
- English
- Spanish
- Japanese
...
- Sandbox deployment
  1. Click on GAO Sandbox button
  2. Click on Next
  3. Select the desired models to use
  4. Select the Hyperscaler to use
  5. Click on Deploy
...
- Sandbox Deployment: Free of charge for limited hours.

- Local Deployment: Approximate monthly cost for main services of supported hyperscaler.

### Azure hyperscaler (Japan East)
- Kubernetes Cluster: 250$ x 2 nodes = 500$/month
- MySQL Database: 100$/month
- ServiceBus Queues: 0,05$ per 1.000.000 operations
- Blobs Storage: 0,02$/GB + 0,05$ per 10.000 operations
- Cache for Redis: 17$/month
- Form Recognizer OCR: 1,50$ per 1.000 pages
- OpenAI Models: depends on use and model (large catalog to choose)

### AWS hyperscaler (Tokyo)
- Kubernetes Cluster: 280$ x 2 nodes = 560$/month
- RDS Database: 110$/month
- SQS Queues: 0,50$ per 1.000.000 operations
- S3 Buckets Storage: 0,025$/GB + 0,005$ per 1.000 operations
- ElastiCache Redis: 19$/month
- Textract OCR: 1,50$ per 1.000 pages (Singapore region, not available in Tokyo)
- Bedrock Models: depends on use and model (large catalog to choose)

## Security Consideration

- Access control, user identification and limits management through API Gateway as entry point with API keys.
- Private kubernetes cluster by environment.
- Private endpoints for Azure resources like Blob Storage.
- Segment temporal data in Blob Storage by user and request.
- Deleting of uploaded documents and temporal data after preprocess.

## Performance Optimization

To ensure optimal performance of the Global RAG system during indexing, retrieval, and response generation, consider the following recommendations:

- **Indexing Optimization**

    Tune chunking parameters: Use window_length and window_overlap settings tailored to the document type. For short FAQs, a small window (e.g., 100–200) may work best, while technical manuals may need longer windows (300–500).

    Remove noise early: Filter out unsupported languages, large images, or tables during preprocessing to speed up indexing and improve retrieval accuracy.

- **Retrieval Optimization**

    Adjust top_k parameter:
    Controls how many candidate chunks are retrieved.
    Low values (e.g., top_k=3–5) reduce latency but risk missing context.
    Higher values (e.g., top_k=10–15) improve recall but increase processing time.
    Recommend tuning based on use case (e.g., legal → higher top_k, FAQs → lower).

    Use similarity_threshold:
    Filter out low-scoring results (e.g., cosine similarity < 0.7) to avoid irrelevant content in the final prompt.

    Combine BM25 and embeddings:
    Hybrid retrieval improves robustness across query types (semantic + keyword).

    Use metadata filtering:
    Narrow search scope by date, category, or document type before vector retrieval.

- **LLM Interaction**

    Match model to task complexity:
    Use small/faster models (e.g., GPT-4o-mini) for summaries or factual Q&A, and larger models (e.g., GPT-4o) for synthesis or reasoning.

    Token budget control:
    Limit max_input_tokens to avoid latency or cost spikes.
    Use prompt trimming logic if input exceeds model limits.

- **Compose Flow Tuning**

    Action combination:
    When using Compose templates, merge compatible actions (e.g., filter + groupby + llm) in a single flow to reduce latency.

    Smart template selection:
    Choose the lightest possible template:

    techhub_llm_model → no retrieval

    techhub_retrieval → no LLM

    techhub_retrieval_reference → full RAG

## Scalability

The components are prepared to scale by queue on demand, you just need to install KEDA in the cluster.

## Monitoring & Logging

- All components have Python logging to write in console all level traces from DEBUG to CRITICAL, level is configurable with the environment variable `LOG_LEVEL`, by default level is set to INFO.
- Also there is optional tracing with Langfuse in the component compose to monitor query-answer with detailed actions executed, for that it's necessary to enable and configure the Langfuse credentials.
- Grafana and Prometheus to internal monitoring of the resources.

## Best Practice

This section provides a set of guidelines, recommended tools, and techniques for building efficient, scalable, and maintainable workflows using the Global RAG platform.

#### Workflow Design Guidelines

Use case–driven design: Create workflows tailored to specific objectives such as summarization, Q&A, or classification. Choose Compose templates accordingly.

Build incrementally: Start with retrieval-only flows and progressively add complexity. Validate each step—retrieval, filtering, prompting—before combining.

Data and metadata consistency: Use standardized metadata fields (e.g., category, year, source) across indexing. Maintain consistent index naming conventions.

#### Recommended Tools and Configuration Strategies

**Compose Templates**  
techhub_llm_model: Use for prompt-only tasks (no retrieval).

techhub_retrieval_reference: Full RAG (retrieval + LLM + references).

techhub_retrieval: Use when generation is not required.

**Chunking**  
window_length: 200–400 tokens for most document types.

window_overlap: 10–20% of chunk size (typically 20–80 tokens).

Avoid overly small or overlapping chunks that increase token usage.

**Retrieval**  
top_k: Recommended range is 5–10, depending on use case and index size.

similarity_threshold: Use values like 0.7–0.8 to filter out weak matches.

Combine scoring methods (e.g., embeddings + BM25) where supported.

**Prompting**  
Use domain-specific system messages (e.g., “You are a legal advisor...”).

Enforce clear instructions and context limits.

Compress or trim inputs when nearing model token limits.

**Semantic enrichment**  
Add business-relevant tags or classifications during indexing to enhance targeting.

**Compose chaining**  
Combine actions (e.g., filter → retrieve → groupby → LLM) into a single Compose call to reduce round trips.

**Response validation**   
Post-process LLM outputs or use validation prompts to ensure accuracy in critical scenarios.


#### Maintenance and Observability

**Template and prompt versioning**  
Store Compose templates and LLM prompts in version-controlled repositories or cloud storage (Azure Blob, Langfuse).

**Monitoring and logging**  
Token usage per call, failure rates and timeouts, ,monitor performance, latency, and cost across multiple providers (Azure, Bedrock, Vertex) to optimize routing and platform selection.
...
In this section it is listed all the endpoints from Compose component.
...
## INFORETRIEVAL API specification
...
This building block has no interface, but it uses Langfuse application for template management. In this section we explain how to use it.

## Langfuse

**Langfuse** serves as the central platform for storing and managing all LLM prompts and templates used by the `llmapi` and `compose` components. It also provides robust version control to track prompt changes over time.

### Accessing Projects in Langfuse

After logging into Langfuse, you'll land on the **Homepage**, which shows all available projects.

![alt text](documentation/media/langfuse/homepage_langfuse.png)

Select the appropriate project (in this case, `TechHub`) to view its internal configuration.

### Inside a Project

Once inside the `TechHub` project, you'll see a dashboard with general information about the project.

![alt text](documentation/media/langfuse/project_screen_langfuse.png)

From here, navigate to the **Prompts** section to access and manage prompts used in different parts of the system.

### Prompt Overview

The **Prompts screen** shows a list of all stored prompts.

![alt text](documentation/media/langfuse/prompts_screen_langfuse.png)

Each prompt has associated metadata and tags that help identify its use, version, and status.

### Prompt Content and Tags

Clicking on a prompt opens the full content, including the prompt body and its tag history.

![alt text](documentation/media/langfuse/prompt_content_langfuse.png)

Each prompt includes the following tags:

- **latest**: Indicates the most recent version of the prompt.
  ![alt text](documentation/media/langfuse/latest_tag_langfuse.png)
- **Component-specific tags**:
  - `llm_template`: Used by the `llmapi` component.
  - `compose_template` or `compose_filter_template`: Used by the `compose` component.
  ![alt text](documentation/media/langfuse/component_tag_langfuse.png)
- **deleted**: Marks the prompt as deleted. Prompts with this tag are **not active** and **cannot be used**, but they are retained for version history.

### Versioning and Changes

Whenever a prompt is updated or marked as deleted (by adding the `deleted` tag) and then created again, a new version is automatically created. Each version is stored along with its associated tags, ensuring complete traceability of changes over time.

![alt text](documentation/media/langfuse/versions_tags_langfuse.png)

### Uploading a Template

You can upload a prompt template using an API request. Here's an example using the **llmapi**:

![alt text](documentation/media/langfuse/upload_prompt_template.png)

Once uploaded, the template will be visible inside Langfuse with its associated content and tags.

![alt text](documentation/media/langfuse/upload_prompt_template_langfuse.png)


### Retrieving a Template

To verify that the template was created and saved correctly we can use the `get_template` endpoint from the `llm_api` using the template's name:

![alt text](documentation/media/langfuse/get_template.png)


### Deleting a Template

Now to delete the template, we can use the  `delete_template` endpoint.

![alt text](documentation/media/langfuse/delete_template.png)

Afterward, the prompt in Langfuse will show the `deleted` tag and will no longer be usable.

![alt text](documentation/media/langfuse/delete_template_langfuse.png)


### Re-uploading a Template

If a template is re-uploaded after deletion—or simply updated—Langfuse creates a **new version** and updates its tags accordingly. This ensures every change is versioned and auditable.

![alt text](documentation/media/langfuse/template_versions_langfuse.png)

# Processing Flow

This section describes the step-by-step execution of the two main pipelines in the Global RAG system: Indexing and Compose. Each flow outlines how data moves through the system, from document ingestion to response generation, enabling flexible, template-driven, and efficient processing.
...
- APIGW
    ...
    ![alt text](documentation/media/genai-apigw-decision-flow.png "Process flow")

- INTEGRATION

    ![alt text](documentation/media/techhubintegration/genai-integration-receiver-decision-flow.png "Process flow")

    ![alt text](documentation/media/techhubintegration/genai-integration-sender-decision-flow.png "Process flow")

- PREPROCESS

    ![alt text](documentation/media/techhubpreprocess/preprocess-start.png "Process flow")

    ![alt text](documentation/media/techhubpreprocess/preprocess-extract.png "Process flow")

    ![alt text](documentation/media/techhubpreprocess/preprocess-ocr.png "Process flow")

    ![alt text](documentation/media/techhubpreprocess/preprocess-end.png "Process flow")

- INFOINDEXING

    ![alt text](documentation/media/techhubgenaiinfoindexing/genai-infoindexing-v3.0.0-decision-flow.png "Process flow")

- FLOWMGMT

    ![alt text](documentation/media/techhubflowmgmt/flowmgmt-checkend.png "Process flow")

    ![alt text](documentation/media/techhubflowmgmt/flowmgmt-infodelete.png "Process flow")

    ![alt text](documentation/media/techhubflowmgmt/flowmgmt-checktimeout.png "Process flow")

## COMPOSE Flow

- COMPOSE

    ![alt text](documentation/media/genai-compose-decision-flow.png "Process flow")

- INFORETRIEVAL

  ![alt text](documentation/media/techhubgenaiinforetrieval/genai-inforetrieval-v2.2.0-inforetrieval-decision-flow.png "Process flow")

- LLMAPI

    ![alt text](documentation/media/techhubgenaillmapi/genai-llmapi-llmapi-decision-flow.png "Process flow")

# Upgrade

Changes to migrate from v3.1 to v4.0:
- Component 'integration-sender' rename environment variables from 'API_...' to 'CORE_...'
- If you are using a custom profile in blob path 'src/integration/search/profiles/' update with repository file 'services/common/client_specific/search/profiles/default.json'
- If you want to activate Langfuse for templates management instead of blob JSONs:
    - Components 'genai-compose', 'genai-llmapi' and 'genai-llmqueue' add environment variables or secret with the keys:
    	LANGFUSE=true
        LANGFUSE_HOST=https://{CHANGEME}
        LANGFUSE_SECRET_KEY=sk-{CHANGEME}
        LANGFUSE_PUBLIC_KEY=pk-{CHANGEME}
    - Deploy a Langfuse selfhosted instance in the same namespace
    - Components 'langfuse-web' and 'langfuse-worker' add environment variables:
        LANGFUSE_INIT_ORG_ID=techhub
        LANGFUSE_INIT_ORG_NAME=Tech Hub
        LANGFUSE_INIT_PROJECT_ID=default
        LANGFUSE_INIT_PROJECT_NAME=Default
        LANGFUSE_INIT_USER_EMAIL=admin@techhub.com
        LANGFUSE_INIT_USER_NAME=admin
        LANGFUSE_INIT_USER_PASSWORD=techhubemeallangfuse
        LANGFUSE_INIT_PROJECT_PUBLIC_KEY=pk-{CHANGEME}
        LANGFUSE_INIT_PROJECT_SECRET_KEY=sk-{CHANGEME}
    - Delete and upload via Langfuse web-ui the current templates from blob paths:
        src/LLM/prompts/
        src/compose/templates/
        src/compose/filter_templates/
- If you want to use models of the new supported Vertex platform:
    - Add models configuration in 'src/integration/search/models_map.json.json'
    - Add models configuration in 'src/ir/conf/models_config.json'
    - Add default model in 'src/ir/conf/default_embedding_models.json' for 'vertex' platform
    - Updated secret to include the new 'vertex' URL for using Gemini models
- If you want to use models of the new supported Tsuzumi platform:
    - Add model configuration in 'src/LLM/conf/models_config.json'
    - Add default model in 'src/LLM/conf/default_llm_models.json' for 'tsuzumi' platform
    - Updated secret to include the new 'tsuzumi' chat completion URL
...
This section outlines common issues that may arise when using the Global RAG system and provides recommended actions to resolve them. It covers problems related to indexing, retrieval, LLM integration, and Compose execution, helping developers and administrators quickly identify and fix misconfigurations or runtime errors. As soon as we get more errors we will add them here.

## Common issues

- Global RAG does answers "I can not answer your question": This usually happens when the query is not related to the retrieved chunks content. Try to reformulate the query, add more context and be very specific and clear while making the query.
...
- 2025-07-02: Refactor of the entire README file.

# References

In Detailed Components more indepth information can be found for compose, indexing, retrieval, llmapi and preprocess

# Licensing

- All used libraries are allowed for commercial use with licences like MIT or Apache (list available in requirements.txt and OSS section).
- Documents uploaded by user are deleted after processing but information is persisted in vectorial database, the user can delete it in any moment.
- Information passes through services like LLM models and OCR in Azure, AWS or GCP platforms.